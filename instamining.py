...
class InstaHashtagMinior:
    ...
    def login(self):
        ...
        id_input.send_keys(input("what is your id?"))
    ...
    def start(self):
        ...
        search_bar = self.browser.find_element(
            By.CSS_SELECTOR, "._aawg > input"
        )
        search_bar.send_keys(self.initial_hashtag)
        WebDriverWait(self.browser, 10).until(
            EC.presence_of_element_located(
                (By.CSS_SELECTOR, ".x12dtdjy > div:nth-child(5)")
            )
        )
        related_keywords = self.browser.find_elements(
            By.CSS_SELECTOR, ".x12dtdjy > div"
        )
...
def hashtag_minor(hashtag):
    hashtags_minor = InstaHashtagMinior(hashtag)
    hashtags_minor.start()
